...
struct URLSessionWebService: WebServiceProtocol {
    
    // MARK: - WebServiceProtocol Properties
    
    private(set) var baseUrl = "https://api.themoviedb.org/3"
   
    // MARK: - Private Properties

    private let apiKey = "90a5acd11cdcfb676199c9476c07cbb9"
    ...
    private func makeUrlString(fromUrlString urlString: String) -> String {
        let apiKeyString = "api_key=\(apiKey)"
        return urlString.contains("?") ? urlString + "&\(apiKeyString)" : urlString + "?\(apiKeyString)"
    }
    
    private func makeRequest(webServiceRequest: WebServiceRequest, completion: ((Result<Data>) -> Void)) -> URLRequest? {
    ...
    private func makeUrlEncode(params: [String: Any]) -> String {
